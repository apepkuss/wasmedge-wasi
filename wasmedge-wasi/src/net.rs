...
macro_rules! wasi_listen_write_impl {
    ($ty:ty, $stream:ty) => {
        impl WasiFile for $ty {
            ...
            fn sock_accept(&mut self, fdflags: FdFlags) -> Result<Box<dyn WasiFile>, Error> {
                let (stream, _) = self.0.accept()?;
                let mut stream = <$stream>::from_cap_std(stream);
                stream.set_fdflags(fdflags)?;
                Ok(Box::new(stream))
            }
            ...
            fn set_fdflags(&mut self, fdflags: FdFlags) -> Result<(), Error> {
                if fdflags == wasmedge_wasi_common::file::FdFlags::NONBLOCK {
...
macro_rules! wasi_stream_write_impl {
    ($ty:ty, $std_ty:ty) => {
        impl WasiFile for $ty {
            ...
            fn sock_shutdown(&mut self, how: SdFlags) -> Result<(), Error> {
...
pub fn get_fd_flags<Socketlike: AsSocketlike>(
    f: Socketlike,
) -> std::io::Result<wasmedge_wasi_common::file::FdFlags> {
    ...
    match rustix::net::recv(f, &mut [], rustix::net::RecvFlags::empty()) {
        Ok(_) => Ok(wasmedge_wasi_common::file::FdFlags::empty()),
        Err(rustix::io::Errno::WOULDBLOCK) => Ok(wasmedge_wasi_common::file::FdFlags::NONBLOCK),
        Err(e) => Err(e.into()),
    }
}
...
///
/// This returns the flags needed to implement [`WasiFile::get_fdflags`].
pub fn is_read_write<Socketlike: AsSocketlike>(f: Socketlike) -> std::io::Result<(bool, bool)> {
