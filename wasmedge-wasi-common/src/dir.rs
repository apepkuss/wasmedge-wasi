...
pub trait WasiDir: Send + Sync {
    ...
    fn open_dir(&self, _symlink_follow: bool, _path: &str) -> Result<Box<dyn WasiDir>, Error> {
        Err(Error::not_supported())
    }
    ...
    fn read_link(&self, _path: &str) -> Result<PathBuf, Error> {
        Err(Error::not_supported())
    }
    ...
    fn get_path_filestat(&self, _path: &str, _follow_symlinks: bool) -> Result<Filestat, Error> {
        Err(Error::not_supported())
    }

    fn rename(&self, _path: &str, _dest_dir: &dyn WasiDir, _dest_path: &str) -> Result<(), Error> {
        Err(Error::not_supported())
    }
